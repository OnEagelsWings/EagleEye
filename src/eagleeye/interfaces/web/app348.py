...
COOKIE = "eagleeye348_session"
CSRF_COOKIE = "eagleeye348_csrf"
...
def _token_file(root: Path) -> Path:
    path = root / "data/security_348/local_session_token"
...
def create_workspace_app348(*, base_dir: str | Path | None = None) -> FastAPI:
    ...
    @app.get("/health")
    def health():
        metrics = ctx.build348.schema_metrics()
        return {"ok": True, "build": BUILD, "schema_baseline": metrics["within_gate"], "tables": metrics["table"], "indexes": metrics["index"], "search_capsule": True, "opsec_intelligence_v2": True, "data_platform": True, "object_store": True, "search_backend": True, "job_engine": True, "network_execution": False, "external_backend_connections_on_boot": 0}
    ...
    def security_login():
        return shell("<div class='card'><h1>EagleEye Build 348</h1><p>Der lokale Workspace wird nur über den geschützten Launcher geöffnet.</p></div>", "EagleEye Login")

    @app.get("/api/build348")
    def api_build348(request: Request):
        require_auth(request)
        return ctx.build348.dashboard()
    ...
    def api_case_search(case_id: str, request: Request, q: str, limit: int = 20):
        ...
        return {"case_id": case_id, "query": q, "results": ctx.build348.search_case(case_id=case_id, query=q, limit=limit)}

    @app.get("/api/cases/{case_id}/jobs")
    def api_case_jobs(case_id: str, request: Request):
        require_auth(request)
        ctx.cases.get_case(case_id)
        return {"case_id": case_id, "jobs": ctx.build348.list_jobs(case_id=case_id)}

    @app.get("/api/cases/{case_id}/capsules")
    def api_capsules(case_id: str, request: Request):
        require_auth(request)
        return {"case_id": case_id, "capsules": ctx.build348.list_capsules(case_id=case_id)}
    ...
    def home(request: Request):
        ...
        gate = ctx.build348.qualified_gate()
        ...
        body = f"<div class='card'><h1>EagleEye · Phase 15 · Build 348</h1><p>Search & Job Infrastructure: lokaler FTS5-Fallindex und persistente, idempotente Job-Queue auf der Phase-15-Datenplattform. Build 348 öffnet selbst keine externen Verbindungen.</p></div><div class='grid'><div class='card'><div class='metric'>{metrics['table']}</div><div>Tabellen</div></div><div class='card'><div class='metric'>{metrics['index']}</div><div>Indizes</div></div><div class='card'><div class='metric'>{len(ctx.build348.list_capsules(limit=500))}</div><div>Search Capsules</div></div><div class='card'><div class='metric'>OFF</div><div>Runtime Network</div></div></div><div class='card'><b>Build Gate:</b> {'PASS' if gate['build_acceptance_ready'] else 'noch nicht belegt'} · <b>Production:</b> nicht freigegeben · <a href='/api/build348'>JSON-Status</a></div><div class='card'><h2>Fälle</h2><table><tbody>{rows}</tbody></table></div><div class='card'><h2>Neuen Fall anlegen</h2><form method='post' action='/cases'><input type='hidden' name='csrf' value='{_esc(csrf_value)}'><label>Titel</label><input name='title' required><label>Auftraggeber</label><input name='client'><label>Zweck</label><textarea name='purpose' required></textarea><label>Rechtsgrundlage</label><input name='legal_basis' required><button>Anlegen</button></form></div>"
        return shell(body)
    ...
    def store_text_object(case_id: str, request: Request, content: str = Form(...), search_run_id: str = Form(""), source_id: str = Form(""), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.build348.ingest_artifact(case_id=case_id, content=content, media_type="text/plain", search_run_id=search_run_id.strip() or None, source_id=source_id.strip() or None, provenance={"ui": "build348_local_text_handoff"})
    ...
    def create_clearnet_capsule(case_id: str, request: Request, egress_hosts: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build348.create_clearnet_capsule(case_id=case_id, egress_hosts=hosts)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def register_darknet_source(case_id: str, request: Request, onion_host: str = Form(...), display_name: str = Form(...), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.cases.get_case(case_id)
        ctx.build348.register_darknet_source(onion_host=onion_host, display_name=display_name, source_class="research")
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def approve_darknet_source(case_id: str, source_id: str, request: Request, rationale: str = Form(...), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.cases.get_case(case_id)
        ctx.build348.review_darknet_source(source_id, decision="approve_read_only", rationale=rationale, reviewer=ctx.actor)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def create_darknet_research(case_id: str, request: Request, query: str = Form(...), source_ids: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build348.create_darknet_research(case_id=case_id, query=query, source_ids=ids, human_approved=True)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def preflight(search_run_id: str, request: Request, url: str = Form(...), source_id: str = Form(""), resolved_ip: str = Form(""), csrf: str = Form(...)):
        ...
        result = ctx.build348.preflight_request(search_run_id, url=url, source_id=source_id or None, resolved_ips=ips, browser_webrtc_disabled=True, dns_via_approved_profile=True)
    ...
    def close_capsule(search_run_id: str, request: Request, csrf: str = Form(...)):
        ...
        if not capsule:
            raise HTTPException(status_code=404, detail="Capsule not found")
        ctx.build348.close_capsule(search_run_id)
    ...
    def api_opsec_v2(search_run_id: str, request: Request):
        require_auth(request)
        return {"search_run_id": search_run_id, "assessments": ctx.build348.assessments(search_run_id)}
    ...
    def local_search_index(case_id: str, request: Request, title: str = Form(...), text: str = Form(...), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.cases.get_case(case_id)
        ctx.build348.index_local_text(case_id=case_id, title=title, text=text, provenance={"ui": "build348_local_index"})
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def case_search(case_id: str, request: Request, q: str):
        ...
        results = ctx.build348.search_case(case_id=case_id, query=q, limit=50)
    ...
    def case_detail(case_id: str, request: Request):
        ...
        capsules = ctx.build348.list_capsules(case_id=case_id, limit=100)
        ...
        counts = {
            ...
            "jobs": len(ctx.build348.list_jobs(case_id=case_id, limit=500)),
            "indexed": len(ctx.build348.search_case(case_id=case_id, query="*", limit=1)) if False else int(ctx.db.one("SELECT COUNT(*) c FROM phase15_search_documents WHERE case_id=?", (case_id,))["c"]),
        }
        ...
        capsule_rows = ""
        for c in capsules:
            decisions = ctx.build348.decisions(c["search_run_id"])
