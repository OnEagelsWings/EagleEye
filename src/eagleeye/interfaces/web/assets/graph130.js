(() => {
  'use strict';
  const root = document.getElementById('graph130');
  if (!root) return;
  ...
  const detail = document.getElementById('graph130-detail');
  const search = document.getElementById('graph130-search');
  const reset = document.getElementById('graph130-reset');
  const fit = document.getElementById('graph130-fit');
  ...
  function showDetail(n){
    detail.innerHTML = `<div class="graph-detail-type">${esc(n.type)}</div><h3>${esc(n.label)}</h3>`+
      `<p>${esc(n.value || '')}</p><dl><dt>Konfidenz</dt><dd>${esc(n.confidence)}</dd><dt>Status</dt><dd>${n.candidate?'candidate_only':'reviewed'}</dd><dt>Quelle</dt><dd>${esc(n.source||'')}</dd><dt>Grad</dt><dd>${esc(n.degree ?? '')}</dd><dt>Betweenness</dt><dd>${esc(n.betweenness ?? '')}</dd><dt>Komponente</dt><dd>${esc(n.component_id ?? '')}</dd><dt>Community</dt><dd>${esc(n.community_id ?? '')}</dd><dt>ID</dt><dd><code>${esc(n.id)}</code></dd></dl>`;
  }
  ...
  function render(){
    ...
    for (const n of nodes){
      ...
      circle.setAttribute('r', Math.min(18, (n.type==='person'?10:n.type==='evidence'?8:6) + Math.sqrt(Math.max(0, Number(n.degree||0))))); circle.setAttribute('fill',color(n.type));
  ...
  function clusteredLayout(w,h){
    const groups=new Map();
    for(const n of nodes){const key=n.community_id||n.component_id||`type:${n.type||'other'}`;if(!groups.has(key))groups.set(key,[]);groups.get(key).push(n);}
    const entries=[...groups.entries()].sort((a,b)=>b[1].length-a[1].length||String(a[0]).localeCompare(String(b[0])));
    const outer=Math.max(180,Math.min(w,h)*.34);
    entries.forEach((entry,gi)=>{
      const members=entry[1],ga=(gi/Math.max(1,entries.length))*Math.PI*2,cx=w/2+Math.cos(ga)*outer,cy=h/2+Math.sin(ga)*outer;
      const local=Math.max(32,18*Math.sqrt(members.length));
      members.forEach((n,i)=>{const a=(i/Math.max(1,members.length))*Math.PI*2+(i%5)*.11;const ring=18+local*Math.sqrt((i+1)/Math.max(1,members.length));n.x=cx+Math.cos(a)*ring;n.y=cy+Math.sin(a)*ring;n.vx=0;n.vy=0;});
    });
  ...
  function layout(){
    ...
    if(nodes.length>350){clusteredLayout(w,h);return;}
    ...
    let steps=0;
    function tick(){
      const alpha=Math.max(.025,1-steps/140);
      ...
      updatePositions(); steps++; if(steps<140) requestAnimationFrame(tick); else fitGraph();
    }
  ...
  fetch(`/api/graph130?case_id=${encodeURIComponent(caseId)}`,{credentials:'same-origin'})
    .then(r=>{if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json();})
    .then(data=>{nodes=data.nodes||[];edges=data.edges||[];document.getElementById('graph130-count').textContent=`${nodes.length} Knoten · ${edges.length} Kanten`;render();layout();})
