...
def create_workspace_app401(*, base_dir: str | Path | None = None) -> FastAPI:
    app = create_workspace_app400(base_dir=base_dir); ctx = app.state.context; team = ctx.team_identity_359; _ = ctx.build401
    app.title = "EagleEye Intelligence Platform – Build 401.0 Security & Qualification Hardening"; app.version = "401.0"
    ...
    @app.get("/health")
    def health401():
        prev = dict(old_health() if callable(old_health) else {"ok": True}); gate = ctx.build401.security_gate(); prev.update({"build": "401.0", "phase": 18, "phase18_builds_completed": 1, "security_qualification_gate": gate["security_gate_pass"], "github_feedback_integrated": True, "next_public_feedback_build": "405.0", "production_release_ready": False}); return prev
    @app.get("/api/build401/security-gate")
    def security_gate(request: Request): auth(request); return ctx.build401.security_gate()
    @app.get("/api/build401/phase18-status")
    def phase18_status(request: Request): auth(request); return ctx.build401.phase18_status()
    return app
