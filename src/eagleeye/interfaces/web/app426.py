...
def create_workspace_app426(*,base_dir=None):
 ...
 def ranked426(case_id:str,request:Request):case_auth(request,case_id,'case.read');return ctx.build426.ranked_case(case_id)
 @app.post('/api/build426/crawler/tasks/{task_id}/priority')
 async def priority426(task_id:str,request:Request):
  row=ctx.db.one('SELECT case_id FROM crawl_task_425 WHERE task_id=?',(task_id,))
  if not row:raise HTTPException(404,'crawl task not found')
