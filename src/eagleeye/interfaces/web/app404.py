...
def create_workspace_app404(*, base_dir: str | Path | None = None) -> FastAPI:
    ...
    @app.get('/health')
    def health404():
        prev=dict(old_health() if callable(old_health) else {'ok':True}); g=ctx.build404.review_gate_status(); prev.update({'build':'404.0','phase':18,'phase18_builds_completed':4,'ai_review_gate':True,'ai_review_gate_pass':g['ai_review_gate_pass'],'open_blocking_ai_findings':g['open_blocking_count'],'next_public_feedback_build':'405.0','production_release_ready':False}); return prev
