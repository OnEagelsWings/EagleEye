...
COOKIE = "eagleeye347_session"
CSRF_COOKIE = "eagleeye347_csrf"
...
def _token_file(root: Path) -> Path:
    path = root / "data/security_347/local_session_token"
...
def create_workspace_app347(*, base_dir: str | Path | None = None) -> FastAPI:
    ...
    @app.get("/health")
    def health():
        metrics = ctx.build347.schema_metrics()
        return {"ok": True, "build": BUILD, "schema_baseline": metrics["within_gate"], "tables": metrics["table"], "indexes": metrics["index"], "search_capsule": True, "opsec_intelligence_v2": True, "data_platform": True, "object_store": True, "network_execution": False, "external_backend_connections_on_boot": 0}
    ...
    def security_login():
        return shell("<div class='card'><h1>EagleEye Build 347</h1><p>Der lokale Workspace wird nur über den geschützten Launcher geöffnet.</p></div>", "EagleEye Login")

    @app.get("/api/build347")
    def api_build347(request: Request):
        require_auth(request)
        return ctx.build347.dashboard()

    @app.get("/api/cases/{case_id}/capsules")
    def api_capsules(case_id: str, request: Request):
        require_auth(request)
        return {"case_id": case_id, "capsules": ctx.build347.list_capsules(case_id=case_id)}
    ...
    def home(request: Request):
        ...
        gate = ctx.build347.qualified_gate()
        ...
        body = f"<div class='card'><h1>EagleEye · Phase 15 · Build 347</h1><p>Data Platform Foundation: konsolidierte Backend-Verträge, lokaler Content-Addressed Object Store sowie opt-in PostgreSQL/S3-Adapter. Build 347 öffnet beim Start keine externen Backend-Verbindungen.</p></div><div class='grid'><div class='card'><div class='metric'>{metrics['table']}</div><div>Tabellen</div></div><div class='card'><div class='metric'>{metrics['index']}</div><div>Indizes</div></div><div class='card'><div class='metric'>{len(ctx.build347.list_capsules(limit=500))}</div><div>Search Capsules</div></div><div class='card'><div class='metric'>OFF</div><div>Runtime Network</div></div></div><div class='card'><b>Build Gate:</b> {'PASS' if gate['build_acceptance_ready'] else 'noch nicht belegt'} · <b>Production:</b> nicht freigegeben · <a href='/api/build347'>JSON-Status</a></div><div class='card'><h2>Fälle</h2><table><tbody>{rows}</tbody></table></div><div class='card'><h2>Neuen Fall anlegen</h2><form method='post' action='/cases'><input type='hidden' name='csrf' value='{_esc(csrf_value)}'><label>Titel</label><input name='title' required><label>Auftraggeber</label><input name='client'><label>Zweck</label><textarea name='purpose' required></textarea><label>Rechtsgrundlage</label><input name='legal_basis' required><button>Anlegen</button></form></div>"
        return shell(body)
    ...
    def store_text_object(case_id: str, request: Request, content: str = Form(...), search_run_id: str = Form(""), source_id: str = Form(""), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.build347.ingest_artifact(case_id=case_id, content=content, media_type="text/plain", search_run_id=search_run_id.strip() or None, source_id=source_id.strip() or None, provenance={"ui": "build347_local_text_handoff"})
    ...
    def create_clearnet_capsule(case_id: str, request: Request, egress_hosts: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build347.create_clearnet_capsule(case_id=case_id, egress_hosts=hosts)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def register_darknet_source(case_id: str, request: Request, onion_host: str = Form(...), display_name: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build347.register_darknet_source(onion_host=onion_host, display_name=display_name, source_class="research")
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def approve_darknet_source(case_id: str, source_id: str, request: Request, rationale: str = Form(...), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.cases.get_case(case_id)
        ctx.build347.review_darknet_source(source_id, decision="approve_read_only", rationale=rationale, reviewer=ctx.actor)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def create_darknet_research(case_id: str, request: Request, query: str = Form(...), source_ids: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build347.create_darknet_research(case_id=case_id, query=query, source_ids=ids, human_approved=True)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def preflight(search_run_id: str, request: Request, url: str = Form(...), source_id: str = Form(""), resolved_ip: str = Form(""), csrf: str = Form(...)):
        ...
        result = ctx.build347.preflight_request(search_run_id, url=url, source_id=source_id or None, resolved_ips=ips, browser_webrtc_disabled=True, dns_via_approved_profile=True)
    ...
    def close_capsule(search_run_id: str, request: Request, csrf: str = Form(...)):
        ...
        if not capsule:
            raise HTTPException(status_code=404, detail="Capsule not found")
        ctx.build347.close_capsule(search_run_id)
    ...
    def api_opsec_v2(search_run_id: str, request: Request):
        require_auth(request)
        return {"search_run_id": search_run_id, "assessments": ctx.build347.assessments(search_run_id)}
    ...
    def case_detail(case_id: str, request: Request):
        ...
        capsules = ctx.build347.list_capsules(case_id=case_id, limit=100)
        ...
        capsule_rows = ""
        for c in capsules:
            decisions = ctx.build347.decisions(c["search_run_id"])
