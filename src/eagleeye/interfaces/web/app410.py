...
def create_workspace_app410(*,base_dir:str|Path|None=None)->FastAPI:
    app=create_workspace_app405(base_dir=base_dir); ctx=install_phase18_410(app.state.context); team=ctx.team_identity_359
    ...
    @app.get('/api/build410/data-fabric/status')
    def fabric_status(request:Request): auth(request); return ctx.build407.data_fabric_status()
    @app.get('/api/build410/federated-search/status')
    def search_status(request:Request): auth(request); return ctx.build408.federated_search_status()
    @app.get('/api/build410/retrieval-quality/status')
    def quality_status(request:Request): auth(request); return ctx.build409.retrieval_quality_status()
    return app
