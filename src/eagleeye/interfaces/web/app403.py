...
def create_workspace_app403(*, base_dir: str | Path | None = None) -> FastAPI:
    ...
    @app.get('/health')
    def health403():
        prev=dict(old_health() if callable(old_health) else {'ok':True}); n=ctx.build403.negative_path_status()
        prev.update({'build':'403.0','phase':18,'phase18_builds_completed':3,'negative_path_framework':n['negative_path_framework_ready'],'negative_path_scenarios':n['scenario_count'],'next_public_feedback_build':'405.0','production_release_ready':False}); return prev
