...
class Build409RetrievalQualityCoverageService:
    ...
    def retrieval_quality_status(self):
        prev=self.build408.federated_search_status(); prev_checks={k:v for k,v in prev['checks'].items() if k!='version_coherent'}; st=self.quality409.status()
        checks={'version_coherent':self.version_status()['coherent'],'build408_security_invariants':all(prev_checks.values()),'network_execution_disabled':st['network_execution'] is False,'truth_determination_disabled':st['truth_determined'] is False,'automatic_evidence_promotion_disabled':st['automatic_evidence_promotion'] is False,'coverage_gap_detection':st['coverage_gap_detection'] is True,'source_diversity_metrics':st['source_diversity_metrics'] is True}
        return {**st,'checks':checks,'retrieval_quality_gate_pass':all(checks.values()),'production_release_ready':False,'feedback_checked_before_build':True,'next_public_feedback_build':'410.0'}
