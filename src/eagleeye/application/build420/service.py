...
class Build420Phase18QualificationService:
 ...
 def qualification_status(self):
  s=self.qualification420.status(); checks={'version_coherent':self.version_status()['coherent'],'qualification_integrity':s['integrity_valid'],'fail_closed':s['qualification_fail_closed'],'deep_review_required':s['full_regression_required'] and s['codex_review_required'] and s['github_ci_required'],'no_forbidden_authority':not any(s[k] for k in ('direct_network_authority','automatic_go_issuance','automatic_evidence_promotion','autonomous_scope_expansion','truth_determined'))}; return {'build':self.BUILD,'checks':checks,'phase18_gate_pass':all(checks.values()),'production_release_ready':False}
