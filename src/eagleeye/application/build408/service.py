...
class Build408FederatedSearchService:
    ...
    def federated_search_status(self):
        prev=self.build407.data_fabric_status(); prev_checks={k:v for k,v in prev['checks'].items() if k!='version_coherent'}; reg=self.registry405.verify_integrity(); integ=self.search408.verify_integrity(); st=self.search408.status(); checks={'version_coherent':self.version_status()['coherent'],'build407_security_invariants':all(prev_checks.values()),'source_registry_integrity':reg['valid'],'source_health_history_integrity':reg.get('health_history_valid') is True,'federated_search_integrity':integ['valid'],'network_execution_disabled':st['network_execution'] is False,'automatic_go_disabled':st['automatic_go'] is False,'automatic_evidence_promotion_disabled':st['automatic_evidence_promotion'] is False,'provenance_required':st['provenance_required'] is True}; return {**st,'checks':checks,'federated_search_gate_pass':all(checks.values()),'production_release_ready':False,'feedback_checked_before_build':True,'next_public_feedback_build':'410.0'}
