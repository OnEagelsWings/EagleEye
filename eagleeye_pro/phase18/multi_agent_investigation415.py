...
class MultiAgentInvestigation415:
    ...
    AGENTS={'lead':('Lead Investigator','coordinates questions and synthesis'),'source':('Source Analyst','maps provenance and source coverage'),'temporal':('Temporal Analyst','examines chronology and temporal gaps'),'relationship':('Relationship Analyst','examines explicit structured relationships'),'challenge':('Red-Team Analyst','challenges assumptions and identifies counterevidence needs')}
    ...
    def run_round(self,*,session_id,identity:Mapping[str,Any]|None=None):
      ...
      c=self._context(p); outputs={'lead':{'focus':'coordinate','questions':c['subquestions'],'recommendation':'route unresolved questions to specialist review'},'source':{'focus':'provenance','retrieval_gap_count':c['retrieval_gaps'],'recommendation':'review source coverage and provenance before collection'},'temporal':{'focus':'chronology','temporal_gap_count':c['temporal_gaps'],'recommendation':'review dated records and explicit chronology gaps'},'relationship':{'focus':'explicit_relationships','unresolved_item_count':c['unresolved_relationship_items'],'recommendation':'review unresolved endpoints without inferring hidden relationships'},'challenge':{'focus':'counteranalysis','recommendation':'seek disconfirming evidence and document alternative explanations','truth_determined':False}}
      ids=[self._add(session_id,aid,'analysis_proposal',payload) for aid,payload in outputs.items()]
