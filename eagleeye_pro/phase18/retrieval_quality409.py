...
class RetrievalQualityCoverage409:
    ...
    def _source_meta(self):
        ...
        for a in self.fabric407.adapters():
            ...
            out[a['source_id']]={'adapter_kind':a['adapter_kind'],'remote':bool(a['remote']),'provenance_class':src.get('provenance_class','unknown'),'health_state':src.get('health_state','unknown')}
        return out
    def assess(self,search_id):
        sess=self.search408.session(search_id); results=self.search408.merged_results(search_id); meta=self._source_meta(); planned=[p['source_id'] for p in sess['source_plan']]; observed=sorted({r['source_id'] for r in results})
        adapter_kinds=sorted({meta.get(s,{}).get('adapter_kind','unknown') for s in observed}); provenance_classes=sorted({meta.get(s,{}).get('provenance_class','unknown') for s in observed}); remote_states={bool(meta.get(s,{}).get('remote')) for s in observed}
        refs=[str(r.get('canonical_ref') or '').strip() for r in results if str(r.get('canonical_ref') or '').strip()]; ref_hosts=[]
        for ref in refs:
            host=ref.split('://',1)[-1].split('/',1)[0].casefold()
            if host: ref_hosts.append(host)
        host_counts=Counter(ref_hosts); duplicate_host_pressure=max(host_counts.values())/len(ref_hosts) if ref_hosts else 0.0
        coverage={'planned_sources':len(planned),'observed_sources':len(observed),'source_coverage_ratio':round(len(observed)/len(planned),6) if planned else 0.0,'adapter_kind_count':len(adapter_kinds),'provenance_class_count':len(provenance_classes),'remote_local_mix':remote_states=={False,True},'canonical_host_count':len(host_counts),'duplicate_host_pressure':round(duplicate_host_pressure,6)}
        gaps=[]
        ...
        quality={'coverage':coverage,'observed_source_ids':observed,'adapter_kinds':adapter_kinds,'provenance_classes':provenance_classes,'gaps':gaps,'coverage_sufficient_for_analysis':not gaps,'truth_determined':False,'truth_probability':None,'evidence_promoted':False,'network_execution':False}
        quality['assessment_hash']=_sha({'search_id':search_id,'quality':quality}); return quality
