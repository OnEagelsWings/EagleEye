...
# Security-relevant mutation surfaces introduced across the governed Phase-17 pipeline.
# Read-only accessors are intentionally excluded: Build 402 focuses on writes, approvals,
# execution authority, evidence promotion, and release/readiness mutations.
SURFACES = (
    ...
    AuthorizationSurface("holdout.external_run.record", "eagleeye_pro.phase17.model_holdout398", "ModelHoldoutEvaluation398", "record_external_model_run", "dossier.review", "global", authorization_method="_authorize_qualification"),
    AuthorizationSurface("holdout.human_review.submit", "eagleeye_pro.phase17.model_holdout398", "ModelHoldoutEvaluation398", "submit_human_review", "dossier.review", "global", authorization_method="_authorize_qualification"),
    AuthorizationSurface("soak.external_bundle.import", "eagleeye_pro.phase17.target_soak399", "TargetEnvironmentSoak399", "import_external_bundle", "dossier.review", "global", authorization_method="_authorize_qualification"),
    AuthorizationSurface("soak.external_session.review", "eagleeye_pro.phase17.target_soak399", "TargetEnvironmentSoak399", "review_external_session", "dossier.review", "global", authorization_method="_authorize_qualification"),
...
class AuthorizationMatrixAudit402:
    ...
    It never grants capability and never invokes mutation methods.  Its purpose is to
    make the authorization contract explicit, inspectable and regression-testable.
    """
    ...
    def matrix(self) -> dict[str, Any]:
        ...
        return {
            ...
            "default_deny": True,
            "rows": rows,
            "mutation_surfaces": len(rows),
            "capabilities": sorted({s.capability for s in SURFACES}),
            "global_or_case_mutations": [s.key for s in SURFACES if s.scope == "global_or_case"],
    ...
    def _source_check(self, surface: AuthorizationSurface) -> dict[str, Any]:
        try:
            ...
            auth_path_present = (
                "governance.authorize" in combined_source
                or "self._authorize(" in method_source
                or "self._auth(" in method_source
            )
            capability_present = surface.capability in combined_source
            # Mutations protected only by case.read are an explicit Build-402 failure.
            mutation_not_read_only = (not surface.mutation) or surface.capability != "case.read"
            return {
                "key": surface.key,
                "source_resolved": True,
                "authorization_path_present": auth_path_present,
                "expected_capability_present": capability_present,
                "mutation_not_case_read_only": mutation_not_read_only,
                "pass": auth_path_present and capability_present and mutation_not_read_only,
            }
            ...
            return {
                ...
                "expected_capability_present": False,
                "mutation_not_case_read_only": surface.capability != "case.read",
                "pass": False,
    ...
    def audit(self) -> dict[str, Any]:
        ...
        global_scopes_review_only = all(
            s.capability == "dossier.review" for s in SURFACES if s.scope in {"global","global_or_case"}
        )
        checks = {
            "all_sources_resolved": all(x["source_resolved"] for x in source_checks),
            "all_mutations_have_governance_path": all(x["authorization_path_present"] for x in source_checks),
            "all_expected_capabilities_present": all(x["expected_capability_present"] for x in source_checks),
            "no_mutation_uses_case_read_only": all(x["mutation_not_case_read_only"] for x in source_checks),
            "read_only_role_denied_all_catalogued_mutations": read_only_denied,
            "global_acceptance_requires_review_capability": global_scopes_review_only,
            "default_deny_declared": matrix["default_deny"] is True,
        }
        return {
            "build": BUILD,
            "policy": POLICY_ID,
            ...
            "authorization_audit_pass": all(checks.values()),
            "fail_closed": True,
    ...
    def role_matrix(self) -> dict[str, Any]:
        ...
        for role in sorted(ROLE_CAPABILITIES):
            ...
            roles[role] = {cap: cap in grants for cap in caps}
        return {
            "build": BUILD,
            "policy": POLICY_ID,
            "capabilities": caps,
            "roles": roles,
            "read_only_has_mutation_capability": any(roles["read_only"].values()),
            "production_release_ready": False,
        }
    ...
    def status(self) -> dict[str, Any]:
        return {
            ...
            "source_level_governance_audit": True,
            "global_scope_included": True,
            "default_deny": True,
            "read_only": True,
            "fail_closed": True,
            "production_release_ready": False,
