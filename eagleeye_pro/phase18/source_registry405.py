...
class SourceRegistryV2405:
    ...
    def _authorized_identity(self, identity, *, source_id, capability='source.review'):
        ...
        if canonical.get('active') is not True:
            raise PermissionError('inactive identity denied')
        # A caller-supplied role is never authoritative.  Require the canonical
        # account to still be active before governance evaluates capabilities.
    ...
    def status(self):
        ...
        return {'build':BUILD,'policy':POLICY_ID,'source_registry_v2':True,'sources':len(rows),'metadata_complete':complete,'health_counts':health,'network_execution':False,'automatic_live_enablement':False,'provenance_required':True,'human_review_required':True,'mutation_authorization_required':True,'canonical_identity_binding':True,'active_identity_required':True,'registry_reconciliation_required':True}
    def verify_integrity(self):
        bad=[]; history_bad=[]
        canonical_ids={str(r['source_id']) for r in self.db.all('SELECT source_id FROM source_registry_sources')}
        v2_ids={str(r['source_id']) for r in self.db.all('SELECT source_id FROM source_registry_v2_405')}
        history_ids={str(r['source_id']) for r in self.db.all('SELECT DISTINCT source_id FROM source_health_history_405')}
        anchor_ids={str(r['source_id']) for r in self.db.all('SELECT source_id FROM source_health_ledger_anchor_408')}
        for sid in sorted(canonical_ids-v2_ids):
            bad.append({'table':'source_registry_v2_405','id':sid,'reason':'missing_registry_row'})
        for sid in sorted(v2_ids-canonical_ids):
            bad.append({'table':'source_registry_v2_405','id':sid,'reason':'not_in_canonical_registry'})
        for sid in sorted(history_ids-v2_ids):
            history_bad.append({'table':'source_health_history_405','id':sid,'reason':'orphaned_history'})
        for sid in sorted(anchor_ids-v2_ids):
            history_bad.append({'table':'source_health_ledger_anchor_408','id':sid,'reason':'orphaned_anchor'})
        for r in self.list_sources():
            ...
            if _sha(payload)!=r['metadata_hash']: bad.append({'table':'source_registry_v2_405','id':r['source_id'],'reason':'metadata_hash'})
        ...
        return {'valid':not violations,'violations':violations,'registry_reconciled':not bad,'registry_violations':bad,'health_history_valid':not history_bad,'health_history_violations':history_bad}
