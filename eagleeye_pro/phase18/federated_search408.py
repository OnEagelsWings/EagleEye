...
class FederatedSearch408:
    ...
    def __init__(self,db,audit,*,fabric407,registry405,actor='local-analyst'):
        self.db=db; self.audit=audit; self.fabric407=fabric407; self.registry405=registry405; self.actor=actor; self._init_schema()
    ...
    def _eligible_sources(self, source_ids:Iterable[str]|None=None):
        wanted=set(str(x) for x in (source_ids or [])); rows=[]
    ...
    def import_results(self,*,search_id,source_id,results,identity:Mapping[str,Any]|None=None):
        sess=self.session(search_id); allowed={p['source_id']:p for p in sess['source_plan']}
        ...
        actor=str((identity or {}).get('user_id') or (identity or {}).get('username') or self.actor); out=[]; seen=set()
        for item in rows:
            item=dict(item or {}); title=_norm_text(item.get('title')); snippet=_norm_text(item.get('snippet')); ref=_norm_text(item.get('canonical_ref') or item.get('origin_ref')); prov=dict(item.get('provenance') or {})
    ...
    def verify_integrity(self):
        ...
        for r in self.db.all('SELECT * FROM federated_search_session_408'):
            d=dict(r); body={'case_id':d['case_id'],'query':d['query_text'],'source_plan':json.loads(d['source_plan_json']),'network_execution':bool(d['network_execution']),'automatic_go':bool(d['automatic_go'])}; rec={**body,'search_id':d['search_id'],'created_by':d['created_by'],'created_at':d['created_at']}
