...
class ConnectorDataFabric407:
    ...
    def _infer_kind(self,row:Mapping[str,Any])->str:
        caps={str(x).lower() for x in (row.get('capabilities') or [])}; prov=str(row.get('provenance_class') or ''); auth=str(row.get('auth_mode') or ''); joined=' '.join(caps)
    ...
    def seed_from_registry(self):
        ...
        for row in self.registry405.list_sources():
            ...
            body={'source_id':row['source_id'],'adapter_kind':kind,'capabilities':caps,'remote':bool(remote),'requires_go':True,'provenance_required':True}; h=_sha(body)
            old=self.db.one('SELECT adapter_hash FROM connector_fabric_adapters_407 WHERE source_id=?',(row['source_id'],))
    ...
    def plan(self,*,case_id,source_id,operation,params:Mapping[str,Any]|None=None,identity:Mapping[str,Any]|None=None):
        ...
        actor=str((identity or {}).get('user_id') or (identity or {}).get('username') or self.actor); metadata_hash=str(src['metadata_hash'])
        body={'case_id':case_id,'source_id':source_id,'adapter_kind':ad['adapter_kind'],'operation':operation,'params':params,'source_metadata_hash':metadata_hash,'requires_go':True,'network_execution':False,'execution_authority':False}
    ...
    def import_result(self,*,plan_id,payload:bytes|str,media_type,provenance:Mapping[str,Any],identity:Mapping[str,Any]|None=None):
        plan=self.get_plan(plan_id); raw=payload.encode('utf-8') if isinstance(payload,str) else bytes(payload)
        ...
        prov=dict(provenance or {}); required=('retrieved_at','origin_ref','collector'); missing=[k for k in required if not str(prov.get(k) or '').strip()]
        ...
        record={'intake_id':ih,'case_id':plan['case_id'],'source_id':plan['source_id'],'adapter_kind':plan['adapter_kind'],'plan_id':plan_id,'media_type':str(media_type or 'application/octet-stream'),'payload_hash':ph,'payload_bytes':len(raw),'provenance':prov,'imported_by':actor,'imported_at':now}; rh=_sha(record)
    ...
    def verify_integrity(self):
        ...
        for r in self.db.all('SELECT * FROM connector_fabric_intake_407'):
            d=dict(r); rec={'intake_id':d['intake_id'],'case_id':d['case_id'],'source_id':d['source_id'],'adapter_kind':d['adapter_kind'],'plan_id':d['plan_id'],'media_type':d['media_type'],'payload_hash':d['payload_hash'],'payload_bytes':d['payload_bytes'],'provenance':json.loads(d['provenance_json']),'imported_by':d['imported_by'],'imported_at':d['imported_at']}
