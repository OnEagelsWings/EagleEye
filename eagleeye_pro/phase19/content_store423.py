...
class ContentStore423:
 ...
 def ingest(self,*,identity,event_id,content,media_type='text/plain',metadata=None,near_threshold=.88):
  ...
  actor=str(identity.get('user_id') or identity.get('username') or self.actor);ev=self.events422.get(event_id);raw=content.encode('utf-8') if isinstance(content,str) else bytes(content);digest=_sha_bytes(raw);existing=self.db.one('SELECT * FROM content_object_423 WHERE sha256=?',(digest,));kind='unique';related='';sim=1.0 if existing else 0.0
