...
class RegistryOrganizationIntegration434:
 ...
 def import_record(self,*,identity,case_id,source_id,event_id,content_id,registry_kind,registry_name,record_key,record,subject_id='',relations=None,test_fixture=False):
  ...
  if self.db.one('SELECT 1 FROM registry_organization_record_434 WHERE case_id=? AND source_id=? AND registry_kind=? AND record_key=?',(case_id,source_id,kind,key)):raise ValueError('registry record already imported in this case')
  ...
  normalized={'legal_name':legal_name,'organization_type':org_type,'jurisdiction':jurisdiction,'record_status':record_status,'aliases':sub.get('aliases',aliases),'identifiers':observation.get('identifiers',[]),'people':observation.get('people',[]),'attributes':observation.get('attributes',attrs),'observation_id':observation['observation_id'],'auto_created_subject':auto_created_subject,'automatic_entity_resolution':False,'source_claims_are_not_verified_facts':True}
