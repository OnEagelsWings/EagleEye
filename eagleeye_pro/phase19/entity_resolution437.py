 ...
 def sync_case(self,*,identity,case_id,include_fixtures=False,min_news_confidence=.60,min_name_similarity=.84,max_entities=500,max_pairs=2000):
  ...
  for i,left in enumerate(ents):
   for right in ents[i+1:]:
    ...
    cmp=self.entity371.analyze_internal(case_id=case_id,left_entity_id=left['resolution_entity_id'],right_entity_id=right['resolution_entity_id'],actor=str(ident['username']));pairs.append(cmp['comparison_id']);classes[cmp['classification']]=classes.get(cmp['classification'],0)+1
