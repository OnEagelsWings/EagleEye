...
def test_contract_and_launcher(tmp_path,monkeypatch):
    ...
    assert 'app439 import create_workspace_app439' in (ROOT/'src/eagleeye/interfaces/web/server.py').read_text()
    assert (ROOT/'BUILD_438_CASE_TEST.md').exists();assert (ROOT/'README_BUILD_438_0.md').exists()
    readme=(ROOT/'README.md').read_text();assert 'EAGLEEYE_PRO_439_0.py' in readme and 'test_build439_integrated.py' in readme
