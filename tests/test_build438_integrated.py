...
def test_contract_and_launcher(tmp_path,monkeypatch):
    ...
    assert 'app438 import create_workspace_app438' in (ROOT/'src/eagleeye/interfaces/web/server.py').read_text()
    assert (ROOT/'BUILD_438_CASE_TEST.md').exists();assert (ROOT/'README_BUILD_438_0.md').exists()
    readme=(ROOT/'README.md').read_text();assert 'EAGLEEYE_PRO_438_0.py' in readme and 'test_build438_integrated.py' in readme
