...
def test_version_and_phase18_gate(tmp_path):
 with AppContext(base_dir=tmp_path) as c:
  assert c.build420.version_status()['coherent']; s=c.build420.qualification_status(); assert s['phase18_gate_pass']; assert not s['production_release_ready']
def test_qualification_runs_fail_closed_and_persists(tmp_path):
 with AppContext(base_dir=tmp_path) as c:
  a=admin(c); q=c.build420.qualify_phase18(identity=a); assert q['report']['qualification_result'] in {'pass','hold'}; assert q['report']['production_release_ready'] is False; assert c.phase18_qualification_420.verify_integrity()['valid']
...
def test_app_launcher(tmp_path):
 ...
 try:
  h=TestClient(app).get('/health').json(); assert h['build']=='420.0' and h['phase18_complete']; assert not h['production_release_ready']; assert '/api/build420/qualification/run' in {r.path for r in app.routes}
