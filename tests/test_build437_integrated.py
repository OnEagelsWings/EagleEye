...
def test_contract_and_launcher(tmp_path,monkeypatch):
 ...
 assert 'app437 import create_workspace_app437' in (ROOT/'src/eagleeye/interfaces/web/server.py').read_text();assert (ROOT/'BUILD_437_CASE_TEST.md').exists();readme=(ROOT/'README.md').read_text();assert 'EAGLEEYE_PRO_437_0.py' in readme and 'test_build437_integrated.py' in readme
