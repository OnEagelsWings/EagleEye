...
def test_invalid_metadata_fails_closed(tmp_path):
    with ctx(tmp_path) as c:
        a=admin(c); sid=c.build405.sources()[0]['source_id']
        with pytest.raises(ValueError): c.source_registry_v2_405.update_metadata(source_id=sid,capabilities=['registry'],auth_mode='password_in_url',rate_limit_policy='respect provider',usage_policy='public terms',provenance_class='primary_public',identity=a)
        with pytest.raises(ValueError): c.source_registry_v2_405.update_metadata(source_id=sid,capabilities=['registry'],auth_mode='none',rate_limit_policy='',usage_policy='public terms',provenance_class='primary_public',identity=a)
        with pytest.raises(KeyError): c.source_registry_v2_405.update_metadata(source_id='unknown.source',capabilities=['x'],auth_mode='none',rate_limit_policy='n/a',usage_policy='public',provenance_class='primary_public',identity=a)

def test_health_is_observation_not_execution_authority(tmp_path):
    with ctx(tmp_path) as c:
        sid=c.build405.sources()[0]['source_id']
        out=c.source_registry_v2_405.record_health(source_id=sid,health_state='healthy',detail='manual/test observation',identity=admin(c))
