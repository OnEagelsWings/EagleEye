...
COOKIE = "eagleeye346_session"
CSRF_COOKIE = "eagleeye346_csrf"
...
def _token_file(root: Path) -> Path:
    path = root / "data/security_346/local_session_token"
...
def create_workspace_app346(*, base_dir: str | Path | None = None) -> FastAPI:
    ...
    @app.get("/health")
    def health():
        metrics = ctx.build346.schema_metrics()
        return {"ok": True, "build": BUILD, "schema_baseline": metrics["within_gate"], "tables": metrics["table"], "indexes": metrics["index"], "search_capsule": True, "opsec_intelligence_v2": True, "network_execution": False}
    ...
    def security_login():
        return shell("<div class='card'><h1>EagleEye Build 346</h1><p>Der lokale Workspace wird nur über den geschützten Launcher geöffnet.</p></div>", "EagleEye Login")

    @app.get("/api/build346")
    def api_build346(request: Request):
        require_auth(request)
        return ctx.build346.dashboard()

    @app.get("/api/cases/{case_id}/capsules")
    def api_capsules(case_id: str, request: Request):
        require_auth(request)
        return {"case_id": case_id, "capsules": ctx.build346.list_capsules(case_id=case_id)}
    ...
    def home(request: Request):
        ...
        gate = ctx.build346.qualified_gate()
        ...
        body = f"<div class='card'><h1>EagleEye · Phase 15 · Build 346</h1><p>Search-by-Search OPSEC: isolierte Search Session Capsules + OPSEC v2. Build 346 öffnet selbst keine externen Netzwerkverbindungen.</p></div><div class='grid'><div class='card'><div class='metric'>{metrics['table']}</div><div>Tabellen</div></div><div class='card'><div class='metric'>{metrics['index']}</div><div>Indizes</div></div><div class='card'><div class='metric'>{len(ctx.build346.list_capsules(limit=500))}</div><div>Search Capsules</div></div><div class='card'><div class='metric'>OFF</div><div>Runtime Network</div></div></div><div class='card'><b>Build Gate:</b> {'PASS' if gate['build_acceptance_ready'] else 'noch nicht belegt'} · <b>Production:</b> nicht freigegeben · <a href='/api/build346'>JSON-Status</a></div><div class='card'><h2>Fälle</h2><table><tbody>{rows}</tbody></table></div><div class='card'><h2>Neuen Fall anlegen</h2><form method='post' action='/cases'><input type='hidden' name='csrf' value='{_esc(csrf_value)}'><label>Titel</label><input name='title' required><label>Auftraggeber</label><input name='client'><label>Zweck</label><textarea name='purpose' required></textarea><label>Rechtsgrundlage</label><input name='legal_basis' required><button>Anlegen</button></form></div>"
        return shell(body)
    ...
    def create_clearnet_capsule(case_id: str, request: Request, egress_hosts: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build346.create_clearnet_capsule(case_id=case_id, egress_hosts=hosts)
    ...
    def register_darknet_source(case_id: str, request: Request, onion_host: str = Form(...), display_name: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build346.register_darknet_source(onion_host=onion_host, display_name=display_name, source_class="research")
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def approve_darknet_source(case_id: str, source_id: str, request: Request, rationale: str = Form(...), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.cases.get_case(case_id)
        ctx.build346.review_darknet_source(source_id, decision="approve_read_only", rationale=rationale, reviewer=ctx.actor)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def create_darknet_research(case_id: str, request: Request, query: str = Form(...), source_ids: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build346.create_darknet_research(case_id=case_id, query=query, source_ids=ids, human_approved=True)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def preflight(search_run_id: str, request: Request, url: str = Form(...), source_id: str = Form(""), resolved_ip: str = Form(""), csrf: str = Form(...)):
        ...
        result = ctx.build346.preflight_request(search_run_id, url=url, source_id=source_id or None, resolved_ips=ips, browser_webrtc_disabled=True, dns_via_approved_profile=True)
    ...
    def close_capsule(search_run_id: str, request: Request, csrf: str = Form(...)):
        ...
        if not capsule:
            raise HTTPException(status_code=404, detail="Capsule not found")
        ctx.build346.close_capsule(search_run_id)
    ...
    def api_opsec_v2(search_run_id: str, request: Request):
        require_auth(request)
        return {"search_run_id": search_run_id, "assessments": ctx.build346.assessments(search_run_id)}
    ...
    def case_detail(case_id: str, request: Request):
        ...
        capsules = ctx.build346.list_capsules(case_id=case_id, limit=100)
        ...
        capsule_rows = ""
        for c in capsules:
            decisions = ctx.build346.decisions(c["search_run_id"])
