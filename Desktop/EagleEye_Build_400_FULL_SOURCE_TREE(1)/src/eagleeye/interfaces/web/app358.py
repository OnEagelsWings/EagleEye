...
COOKIE = "eagleeye358_session"
CSRF_COOKIE = "eagleeye358_csrf"
...
def _token_file(root: Path) -> Path:
    path = root / "data/security_358/local_session_token"
...
def create_workspace_app358(*, base_dir: str | Path | None = None) -> FastAPI:
    ...
    @app.get("/health")
    def health():
        metrics = ctx.build358.schema_metrics()
    ...
    def security_login():
        return shell("<div class='card'><h1>EagleEye Build 358</h1><p>Der lokale Workspace wird nur über den geschützten Launcher geöffnet.</p></div>", "EagleEye Login")

    @app.get("/api/build358")
    def api_build352(request: Request):
        require_auth(request)
        return ctx.build358.dashboard()
    ...
    def api_case_ai_waves(case_id: str, request: Request):
        ...
        return {"case_id": case_id, "waves": ctx.build358.investigation_research_waves(case_id)}
    ...
    def ai_wave_evaluate(case_id: str, request: Request, csrf: str = Form(...)):
        ...
        ctx.build358.evaluate_investigation_wave(case_id=case_id, allow_followup=True)
    ...
    def api_case_search(case_id: str, request: Request, q: str, limit: int = 20):
        ...
        return {"case_id": case_id, "query": q, "results": ctx.build358.search_case(case_id=case_id, query=q, limit=limit)}

    @app.get("/api/cases/{case_id}/jobs")
    def api_case_jobs(case_id: str, request: Request):
        require_auth(request)
        ctx.cases.get_case(case_id)
        return {"case_id": case_id, "jobs": ctx.build358.list_jobs(case_id=case_id)}

    @app.get("/api/jobs/{job_id}/frontier")
    def api_job_frontier(job_id: str, request: Request):
        require_auth(request)
        try: return ctx.build358.frontier_checkpoint(job_id)
    ...
    def api_case_images(case_id: str, request: Request):
        require_auth(request)
        ctx.cases.get_case(case_id)
        return {"case_id": case_id, "images": ctx.build358.image_assets(case_id=case_id)}

    @app.get("/api/images/{media_id}")
    def api_image(media_id: str, request: Request):
        require_auth(request)
        try: return ctx.build358.image_asset(media_id)
    ...
    def api_image_similarity(media_id: str, request: Request):
        require_auth(request)
        try: return ctx.build358.analyze_similarity(media_id)
        except KeyError: raise HTTPException(status_code=404, detail="Image not found")
    ...
    def api_case_image_links(case_id: str, request: Request):
        require_auth(request)
        ctx.cases.get_case(case_id)
        return {"case_id": case_id, "links": ctx.build358.similarity_links(case_id=case_id)}

    @app.get("/api/cases/{case_id}/geo-links")
    def api_case_geo_links(case_id: str, request: Request):
        require_auth(request)
        ctx.cases.get_case(case_id)
        return {"case_id": case_id, "links": ctx.build358.geolocation_links(case_id=case_id)}

    @app.get("/api/images/{media_id}/visual-context")
    def api_image_visual_context(media_id: str, request: Request):
        require_auth(request)
        try: return ctx.build358.visual_context.context_for_media(media_id)
        except KeyError: raise HTTPException(status_code=404, detail="Image not found")
    ...
    async def api_image_geo_analyze(media_id: str, request: Request):
        ...
        try:
            return ctx.build358.analyze_visual_geolocation(media_id, cues=body.get("cues") or [], include_source_context=bool(body.get("include_source_context", True)), include_landmark_references=bool(body.get("include_landmark_references", True)), human_approved=True)
        except KeyError: raise HTTPException(status_code=404, detail="Image not found")
    ...
    async def api_landmark_reference(media_id: str, request: Request):
        ...
        if not bool(body.get("human_approved")):
            raise HTTPException(status_code=403, detail="Explicit human approval required")
        try:
            return ctx.build358.register_landmark_reference(media_id=media_id, label=str(body.get("label") or ""), city=str(body.get("city") or ""), country=str(body.get("country") or ""), latitude=body.get("latitude"), longitude=body.get("longitude"), human_approved=True)
        except KeyError: raise HTTPException(status_code=404, detail="Image not found")
        except (ValueError, PermissionError) as exc: raise HTTPException(status_code=400, detail=str(exc))
    ...
    async def upload_image(case_id: str, request: Request, image: UploadFile = File(...), search_run_id: str = Form(""), source_id: str = Form(""), csrf: str = Form(...)):
        ...
        try:
            ctx.build358.ingest_image(case_id=case_id, content=data, declared_media_type=image.content_type or "", filename=image.filename or "", search_run_id=search_run_id.strip() or None, source_id=source_id.strip() or None, provenance={"ui": "build358_image_upload"})
    ...
    def api_capsules(case_id: str, request: Request):
        require_auth(request)
        return {"case_id": case_id, "capsules": ctx.build358.list_capsules(case_id=case_id)}

    @app.get("/api/cases/{case_id}/crawler/runs")
    def api_crawler_runs(case_id: str, request: Request):
        require_auth(request)
        ctx.cases.get_case(case_id)
        return {"case_id": case_id, "runs": ctx.build358.crawl_runs(case_id=case_id)}

    @app.get("/api/crawler/sources")
    def api_crawler_sources(request: Request):
        require_auth(request)
        return {"sources": ctx.build358.crawler_sources()}

    @app.post("/api/cases/{case_id}/voice/propose")
    async def api_voice_propose(case_id: str, request: Request):
        require_auth(request); ctx.cases.get_case(case_id); body=await request.json()
        try: return ctx.build358.voice_propose(case_id=case_id, transcript=str(body.get("transcript") or ""), mode=str(body.get("mode") or "command"), source="workspace_transcript")
        except (ValueError, PermissionError) as exc: raise HTTPException(status_code=400, detail=str(exc))
    ...
    async def api_voice_transcribe(case_id: str, request: Request, audio: UploadFile = File(...), mode: str = Form("command"), human_started: str = Form("false"), language: str = Form("de")):
        ...
        try: return ctx.build358.voice_transcribe(case_id=case_id,audio=data,media_type=audio.content_type or "",language=language,human_started=str(human_started).casefold()=="true",mode=mode)
        except (ValueError, PermissionError) as exc: raise HTTPException(status_code=400, detail=str(exc))

    @app.post("/api/voice/{intent_id}/execute")
    async def api_voice_execute(intent_id: str, request: Request):
        require_auth(request); body=await request.json()
        try: return ctx.build358.voice_execute(intent_id=intent_id,confirmed=bool(body.get("confirmed")),edited_transcript=body.get("edited_transcript"))
    ...
    def api_voice_interactions(case_id: str, request: Request):
        require_auth(request); ctx.cases.get_case(case_id); return {"case_id":case_id,"interactions":ctx.build358.voice_interactions(case_id)}
    ...
    def home(request: Request):
        ...
        gate = ctx.build358.qualified_gate()
    ...
    def crawler_source(case_id: str, request: Request, csrf: str = Form(...), display_name: str = Form(...), seed_url: str = Form(...), terms_ref: str = Form(...), sitemap_url: str = Form(""), archive_seed_url: str = Form("")):
        require_csrf(request, csrf); ctx.cases.get_case(case_id)
        ctx.build358.register_crawler_source(display_name=display_name, seed_urls=[seed_url], terms_ref=terms_ref, sitemap_urls=[sitemap_url.strip()] if sitemap_url.strip() else [], archive_seed_urls=[archive_seed_url.strip()] if archive_seed_url.strip() else [])
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def official_connector(case_id: str, request: Request, csrf: str = Form(...), connector_key: str = Form(...), identifier: str = Form(...)):
        require_csrf(request, csrf); ctx.cases.get_case(case_id)
        try:
            result = ctx.build358.register_official_source(connector_key, identifier)
        except PermissionError:
            # Authenticated providers remain plan-only; expose the plan without storing credentials.
            result = {"plan": ctx.build358.connector_source_plan(connector_key, identifier)}
    ...
    def crawler_review(case_id: str, source_id: str, request: Request, csrf: str = Form(...), rationale: str = Form(...)):
        require_csrf(request, csrf); ctx.cases.get_case(case_id)
        ctx.build358.review_crawler_source(source_id, decision="approve_read_only", rationale=rationale, reviewer=ctx.actor)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def crawler_enqueue(case_id: str, source_id: str, request: Request, csrf: str = Form(...)):
        require_csrf(request, csrf); ctx.cases.get_case(case_id)
        ctx.build358.enqueue_crawl(case_id=case_id, source_id=source_id)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def store_text_object(case_id: str, request: Request, content: str = Form(...), search_run_id: str = Form(""), source_id: str = Form(""), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.build358.ingest_artifact(case_id=case_id, content=content, media_type="text/plain", search_run_id=search_run_id.strip() or None, source_id=source_id.strip() or None, provenance={"ui": "build358_crawler_handoff"})
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def create_clearnet_capsule(case_id: str, request: Request, egress_hosts: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build358.create_clearnet_capsule(case_id=case_id, egress_hosts=hosts)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def register_darknet_source(case_id: str, request: Request, onion_host: str = Form(...), display_name: str = Form(...), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.cases.get_case(case_id)
        ctx.build358.register_darknet_source(onion_host=onion_host, display_name=display_name, source_class="research")
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def approve_darknet_source(case_id: str, source_id: str, request: Request, rationale: str = Form(...), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.cases.get_case(case_id)
        ctx.build358.review_darknet_source(source_id, decision="approve_read_only", rationale=rationale, reviewer=ctx.actor)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def create_darknet_research(case_id: str, request: Request, query: str = Form(...), source_ids: str = Form(...), csrf: str = Form(...)):
        ...
        ctx.build358.create_darknet_research(case_id=case_id, query=query, source_ids=ids, human_approved=True)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def preflight(search_run_id: str, request: Request, url: str = Form(...), source_id: str = Form(""), resolved_ip: str = Form(""), csrf: str = Form(...)):
        ...
        result = ctx.build358.preflight_request(search_run_id, url=url, source_id=source_id or None, resolved_ips=ips, browser_webrtc_disabled=True, dns_via_approved_profile=True)
    ...
    def close_capsule(search_run_id: str, request: Request, csrf: str = Form(...)):
        ...
        if not capsule:
            raise HTTPException(status_code=404, detail="Capsule not found")
        ctx.build358.close_capsule(search_run_id)
    ...
    def api_opsec_v2(search_run_id: str, request: Request):
        require_auth(request)
        return {"search_run_id": search_run_id, "assessments": ctx.build358.assessments(search_run_id)}
    ...
    def local_search_index(case_id: str, request: Request, title: str = Form(...), text: str = Form(...), csrf: str = Form(...)):
        require_csrf(request, csrf)
        ctx.cases.get_case(case_id)
        ctx.build358.index_local_text(case_id=case_id, title=title, text=text, provenance={"ui": "build358_local_index"})
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def case_search(case_id: str, request: Request, q: str):
        ...
        results = ctx.build358.search_case(case_id=case_id, query=q, limit=50)
    ...
    def case_detail(case_id: str, request: Request):
        ...
        capsules = ctx.build358.list_capsules(case_id=case_id, limit=100)
        ...
        counts = {
            ...
            "jobs": len(ctx.build358.list_jobs(case_id=case_id, limit=500)),
            "indexed": len(ctx.build358.search_case(case_id=case_id, query="*", limit=1)) if False else int(ctx.db.one("SELECT COUNT(*) c FROM phase15_search_documents WHERE case_id=?", (case_id,))["c"]),
        }
        ...
        capsule_rows = ""
        for c in capsules:
            decisions = ctx.build358.decisions(c["search_run_id"])
    ...
    def api_ai_crawler_monitor(case_id: str, request: Request):
        require_auth(request); ctx.cases.get_case(case_id)
        return ctx.build358.monitor_investigation_crawler(case_id)
    ...
    def ai_intake(case_id: str, request: Request, objective: str = Form(...), key_questions: str = Form(""), scope_notes: str = Form(""), csrf: str = Form(...)):
        ...
        ctx.build358.create_investigation_intake(case_id=case_id, objective=objective, key_questions=questions, scope_notes=scope_notes)
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def ai_go(case_id: str, request: Request, go: str = Form(...), csrf: str = Form(...)):
        require_csrf(request, csrf); ctx.cases.get_case(case_id)
        try:
            ctx.build358.start_investigation_go(case_id=case_id, go=go)
        except (ValueError, PermissionError) as exc:
            raise HTTPException(status_code=400, detail=str(exc))
        return RedirectResponse(f"/cases/{case_id}", status_code=303)
    ...
    def ai_tick(case_id: str, request: Request, csrf: str = Form(...)):
        require_csrf(request, csrf); ctx.cases.get_case(case_id)
        ctx.build358.investigation_supervisor_tick(case_id=case_id)
    ...
    def ai_briefing(case_id: str, request: Request):
        ...
        dossier = ctx.build358.latest_investigation_dossier(case_id)
        if not dossier:
            dossier = ctx.build358.build_investigation_dossier(case_id=case_id)
