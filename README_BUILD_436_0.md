...
The OPSEC gate verifies that correlated onion material retains the Tor/public-source contract: `tor_public` acquisition, quarantined provenance, no destination authentication, no forms and no scope expansion. A failed OPSEC contract is surfaced as a blocker and prevents the run from being marked ready for analyst review. Build 436 never grants active operational-follow-up authority, even when all OPSEC checks pass.
